...
class AnimeQ : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        println("[AnimeQ] 🎬 Iniciando extração de links para: $data")
        println("[AnimeQ] 📊 isCasting: $isCasting")
        
        val result = AnimeQVideoExtractor.extractVideoLinks(data, callback = callback)
        
        if (result) {
            println("[AnimeQ] ✅ Extração concluída com SUCESSO!")
        } else {
            println("[AnimeQ] ❌ Extração FALHOU!")
        }
